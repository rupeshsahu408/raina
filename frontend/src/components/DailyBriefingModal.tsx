          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-[#5c4ff6]">
        ...
        <div className="bg-white">
            ...
            {CATEGORIES.filter(cat => data.categories[cat.key].count > 0).map(cat => {
              ...
              return (
                <div key={cat.key} className={`transition-colors ${isExpanded ? cat.bg : "bg-white hover:bg-slate-50"}`}>
                  ...
                  >
                    <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${cat.iconBg}`}>
                      {cat.icon}
                    </div>
                    ...
                    <div className="px-5 pb-3 space-y-2">
                      {category.preview.map(email => (
                        <div key={email.id} className={`flex items-start gap-2.5 rounded-xl border ${cat.border} bg-white px-3 py-2.5`}>
                          <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] font-black text-white mt-0.5 ${cat.iconBg.split(" ")[0].replace("bg-", "bg-").replace("100", "400")}`}
          ...
          <div className="mx-4 mb-4 mt-3 rounded-2xl bg-gradient-to-r from-indigo-50 to-violet-50 border border-indigo-100 px-4 py-3.5">
            <div className="flex items-start gap-3">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#5c4ff6] text-white mt-0.5">
