...
function BarFill({ pct, color }: { pct: number; color: string }) {
  return (
    <div className="relative h-2 w-full rounded-full bg-gray-50 overflow-hidden">
    ...
    <div className="min-h-screen bg-white text-[#1d2226]">
      ...
      <header className="relative z-10 border-b border-white/[0.07] bg-gray-100 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <button onClick={fetchAnalytics} className="rounded-full border border-white/[0.08] px-3 py-1.5 text-[11px] text-gray-400 hover:text-white hover:border-gray-300 transition">
            Refresh
          </button>
        </div>
      </header>
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
        {loading ? (
          ...
          <>
            {/* Summary Stats */}
            <section>
              <h2 className="text-xs font-bold uppercase tracking-[0.15em] text-gray-400 mb-4">Overview</h2>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
                ...
                <StatCard
                  ...
                  accent={data.avgTimeToHireDays !== null ? "text-amber-400" : "text-gray-400"}
                />
              </div>
            </section>
            ...
            {/* Pipeline Funnel */}
            <section>
              <h2 className="text-xs font-bold uppercase tracking-[0.15em] text-gray-400 mb-4">Pipeline Funnel — Stage Distribution</h2>
              <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6">
                <div className="space-y-4">
                  {data.funnelDropoff.map(({ stage, count, dropoffPct }) => (
                    <div key={stage} className="flex items-center gap-4">
                      <span className={`w-20 shrink-0 text-xs font-medium capitalize ${STAGE_TEXT[stage] ?? "text-gray-500"}`}>{stage}</span>
                      <div className="flex-1">
                        <BarFill pct={dropoffPct} color={STAGE_COLORS[stage] ?? "bg-zinc-500"} />
                      </div>
                      <span className="w-20 text-right text-xs text-gray-400">{count} <span className="text-zinc-700">({dropoffPct}%)</span></span>
                    </div>
                  ))}
                ...
                {total === 0 && (
                  <p className="text-center text-sm text-gray-400 py-8">No candidates yet. Add candidates to jobs to see funnel data.</p>
                )}
              </div>
            </section>
            ...
            {/* Source Quality */}
            <section>
              <h2 className="text-xs font-bold uppercase tracking-[0.15em] text-gray-400 mb-4">Source Quality</h2>
              <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6">
                {data.sourceBreakdown.length > 0 ? (
                  <div className="space-y-3">
                    {data.sourceBreakdown.map(({ source, count, pct }) => (
                      <div key={source} className="flex items-center gap-4">
                        <span className="w-36 shrink-0 text-xs text-gray-500 truncate">{source}</span>
                        <div className="flex-1">
                          <BarFill pct={pct} color="bg-indigo-500" />
                        </div>
                        <span className="w-20 text-right text-xs text-gray-400">{count} <span className="text-zinc-700">({pct}%)</span></span>
                      </div>
                    ))}
                  </div>
                ) : (
            ...
            <section>
              <div className="flex items-center gap-3 mb-4">
                <h2 className="text-xs font-bold uppercase tracking-[0.15em] text-gray-400">Bias Detection Report</h2>
              ...
              <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] p-6">
                {!hasBiasData ? (
                  <div className="space-y-3">
                    <p className="text-sm text-gray-500">Bias detection requires demographic data.</p>
                    ...
                    <div className="mt-4 rounded-2xl border border-white/[0.05] bg-white/[0.02] p-4 space-y-1">
                      <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">What this report will show when data is available:</p>
                  ...
                  <div className="grid gap-6 sm:grid-cols-2">
                    {data.genderBreakdown.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Gender Distribution</p>
                        <div className="space-y-2">
                          {data.genderBreakdown.map(({ gender, count, pct }) => (
                            <div key={gender} className="flex items-center gap-3">
                              <span className="w-20 shrink-0 text-xs text-gray-500 capitalize">{gender}</span>
                              <div className="flex-1"><BarFill pct={pct} color="bg-violet-500" /></div>
                              <span className="w-16 text-right text-xs text-gray-400">{count} ({pct}%)</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {data.ageBreakdown.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Age Range Distribution</p>
                        <div className="space-y-2">
                          {data.ageBreakdown.map(({ ageRange, count, pct }) => (
                            <div key={ageRange} className="flex items-center gap-3">
                              <span className="w-20 shrink-0 text-xs text-gray-500">{ageRange}</span>
                    ...
                    {Object.keys(data.biasStageData).length > 0 && (
                      <div className="sm:col-span-2">
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Gender by Pipeline Stage</p>
                        <div className="overflow-x-auto">
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="border-b border-white/[0.06]">
                                <th className="text-left text-gray-400 pb-2 pr-4 font-medium">Stage</th>
                                {Array.from(new Set(Object.values(data.biasStageData).flatMap(g => Object.keys(g)))).map(g => (
                                  <th key={g} className="text-right text-gray-400 pb-2 px-2 font-medium capitalize">{g}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
                              {Object.entries(data.biasStageData).map(([stage, gMap]) => (
                                <tr key={stage} className="border-b border-white/[0.04]">
                                  <td className={`py-2 pr-4 font-medium capitalize ${STAGE_TEXT[stage] ?? "text-gray-500"}`}>{stage}</td>
                                  {Array.from(new Set(Object.values(data.biasStageData).flatMap(g => Object.keys(g)))).map(g => (
                                    <td key={g} className="py-2 px-2 text-right text-gray-500">{gMap[g] ?? 0}</td>
                                  ))}
                                </tr>
                              ))}
                            </tbody>
            ...
            {data.jobStats.length > 0 && (
              <section>
                <h2 className="text-xs font-bold uppercase tracking-[0.15em] text-gray-400 mb-4">Per-Job Breakdown</h2>
                <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-white/[0.07] text-[11px] uppercase tracking-wide text-gray-400">
                        ...
                        {data.jobStats.map(j => (
                          <tr key={j.jobId} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition">
                            <td className="px-5 py-3">
                              ...
                              {j.department && <p className="text-[11px] text-gray-400">{j.department}</p>}
                            </td>
                            <td className="px-4 py-3 text-center">
                              <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase border ${
                                ...
                                : "bg-zinc-500/15 text-gray-500 border-zinc-500/20"
                              }`}>{j.status}</span>
                            </td>
                            <td className="px-4 py-3 text-center text-gray-600">{j.totalCandidates}</td>
                            <td className="px-4 py-3 text-center">
                            ...
                            <td className="px-5 py-3 text-right text-[11px] text-gray-400">
