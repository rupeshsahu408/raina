...
function renderContent(content: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("## ")) {
      ...
      elements.push(
        <p key={key++} className="mt-5 font-semibold text-zinc-200">
          {line.slice(2, -2)}
        </p>
      );
      i++;
      ...
      elements.push(
        <ul key={key++} className="mt-4 space-y-2 pl-5 list-disc">
          {items.map((item, idx) => {
            ...
            if (boldMatch) {
              return (
                <li key={idx} className="text-zinc-400 text-sm leading-7">
                  <strong className="text-zinc-200">{boldMatch[1]}:</strong>
                  {boldMatch[2]}
                </li>
              );
            }
            return <li key={idx} className="text-zinc-400 text-sm leading-7">{item}</li>;
          })}
        </ul>
      );
    } else {
      ...
      while ((match = boldRegex.exec(line)) !== null) {
        ...
        parts.push(<strong key={match.index} className="text-zinc-200">{match[1]}</strong>);
        lastIndex = boldRegex.lastIndex;
      }
      if (hasMatch) {
        if (lastIndex < line.length) parts.push(line.slice(lastIndex));
        elements.push(<p key={lineKey} className="mt-4 text-sm leading-8 text-zinc-400">{parts}</p>);
      } else {
        elements.push(<p key={lineKey} className="mt-4 text-sm leading-8 text-zinc-400">{line}</p>);
      }
      i++;
    }
  }
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_300px] lg:items-start">
          ...
          <div>
            ...
            <header className="mt-6">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-5 pb-8 border-b border-white/10">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br text-white text-sm font-bold ${post.authorGradient}`}>
                    {post.authorInitials}
                  </div>
                  <div>
            ...
            <div className="mt-12 pt-8 border-t border-white/10">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-600 mb-4">Tags</p>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <span key={tag} className="rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs text-zinc-400 transition hover:border-white/20 hover:text-zinc-300">
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            <div className="mt-8 flex items-center gap-3">
              ...
              <Link href="/blog" className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs text-zinc-400 transition hover:bg-white/10 hover:text-white">
                More Articles
              </Link>
            </div>
          </div>
          ...
          <aside className="space-y-5 lg:sticky lg:top-20">
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-500 mb-4">Browse Categories</p>
                ...
                {categories.map((c) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm transition hover:bg-white/[0.05] ${c.slug === post.category ? `${c.bg} ${c.color} font-semibold` : "text-zinc-400 hover:text-white"}`}
                    >
            ...
            {related.length > 0 && (
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-500 mb-4">Related Articles</p>
                <div className="space-y-4">
                  {related.map((rp) => {
                    const rc = categories.find((c) => c.slug === rp.category);
                    return (
                      <Link key={rp.slug} href={`/blog/${rp.slug}`} className="group block">
                        <p className="text-sm font-semibold text-zinc-300 group-hover:text-white transition-colors leading-snug">
                          {rp.title}
                        </p>
        ...
        {posts.filter((p) => p.slug !== post.slug).length > 0 && (
          <div className="mt-20">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/blog" className="text-xs text-purple-400 hover:text-purple-300 transition flex items-center gap-1">
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {posts.filter((p) => p.slug !== post.slug).slice(0, 3).map((rp) => {
                const rc = categories.find((c) => c.slug === rp.category);
                return (
                  <Link key={rp.slug} href={`/blog/${rp.slug}`} className="group rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl transition hover:border-white/20 hover:bg-white/[0.06]">
                    {rc && (
                    ...
                    <h3 className="text-sm font-semibold text-zinc-200 group-hover:text-white transition-colors leading-snug">{rp.title}</h3>
                    <p className="mt-2 text-xs leading-5 text-zinc-500 line-clamp-2">{rp.excerpt}</p>
