  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 selection:bg-purple-500/30">
      <div className="fixed inset-0 z-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(99,102,241,0.13),transparent_55%)]" />
        <div className="absolute top-[-10%] right-[-8%] w-[40%] h-[40%] rounded-full bg-indigo-900/15 blur-[130px]" />
        <div className="absolute bottom-[5%] left-[-8%] w-[35%] h-[35%] rounded-full bg-purple-900/10 blur-[120px]" />
      </div>
      ...
      <main className="relative z-10">
          ...
          <div className="relative w-full max-w-3xl">
            ...
            <div className="relative mt-8">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-white/10 bg-white/[0.07] py-4 pl-12 pr-12 text-base text-white placeholder-zinc-600 backdrop-blur-xl outline-none transition focus:border-indigo-500/50 focus:bg-white/[0.09] focus:ring-2 focus:ring-indigo-500/20"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
                  className="absolute inset-y-0 right-4 flex items-center text-zinc-500 hover:text-zinc-300 transition"
                >
            ...
            {search && (
              <div className="mt-3 text-xs text-zinc-500">
                {hasResults ? (
                  <span>{filteredFaqs.length} answer{filteredFaqs.length !== 1 ? "s" : ""} and {filteredCategories.length} categor{filteredCategories.length !== 1 ? "ies" : "y"} found for "<span className="text-zinc-300">{search}</span>"</span>
                ) : (
                  <span>No results for "<span className="text-zinc-300">{search}</span>" — try different keywords or <Link href="/contact" className="text-indigo-400 hover:underline">contact support</Link>.</span>
                )}
              </div>
            )}
              ...
              <div className="mt-5 flex flex-wrap justify-center gap-2">
                {["Setting up WhatsApp AI", "Ivana AI prompts", "Cancel subscription", "API key", "Webhook error"].map((q) => (
                  <button
                    key={q}
                    onClick={() => setSearch(q)}
                    className="rounded-full border border-white/10 bg-white/5 px-3.5 py-1.5 text-xs text-zinc-400 transition hover:border-white/20 hover:text-zinc-200"
                  >
                    {q}
                  </button>
                ))}
        ...
        {!search && (
          <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
            <div className="mb-6 flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-600">{popularArticles.length} articles</span>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {popularArticles.map((a, i) => (
                <div
                  key={i}
                  className="group flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur-xl transition hover:border-white/20 hover:bg-white/[0.06] cursor-pointer"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-zinc-200 group-hover:text-white transition-colors truncate">{a.title}</p>
                    <p className="text-xs text-zinc-600 mt-0.5">{a.category} · {a.time} read</p>
                  </div>
                  <ArrowRightIcon className="h-4 w-4 text-zinc-600 group-hover:text-zinc-400 transition shrink-0" />
                </div>
              ))}
            </div>
          </section>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">Select a topic to explore articles in that area</p>
              </div>
              {activeCategory && (
                <button
                  onClick={() => setActiveCategory(null)}
                  className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-zinc-400 transition hover:text-white"
                >
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {filteredCategories.map((cat) => (
                <button
                  ...
                  className={`group text-left rounded-[1.5rem] border p-5 backdrop-blur-xl transition-all duration-200 hover:scale-[1.02] ${cat.bg} ${cat.border} ${activeCategory === cat.id ? "ring-2 ring-white/20 scale-[1.02]" : ""}`}
                >
                  <div className={`flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-black/30 mb-4 ${cat.color}`}>
                    {cat.icon}
                  </div>
                  <div className="flex items-start justify-between gap-2">
                    <p className={`font-semibold text-sm ${cat.color}`}>{cat.label}</p>
                    <span className="shrink-0 rounded-full border border-white/10 bg-black/20 px-2 py-0.5 text-[10px] text-zinc-500">{cat.count}</span>
                  </div>
                  <p className="mt-1.5 text-xs leading-5 text-zinc-500">{cat.description}</p>
                  <div className="mt-4 border-t border-white/5 pt-3 space-y-1.5">
                    {cat.articles.slice(0, 3).map((a, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-zinc-500 group-hover:text-zinc-400 transition">
                        <span className={`h-1 w-1 rounded-full shrink-0 ${cat.dot}`} />
                        <span className="truncate">{a}</span>
                      </div>
                    ))}
                    {cat.articles.length > 3 && (
                      <p className="text-xs text-zinc-600 pl-3">+{cat.articles.length - 3} more</p>
                    )}
                  </div>
                </button>
              ))}
            ...
            {filteredCategories.length === 0 && search && (
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-8 text-center mt-4">
                <p className="text-sm text-zinc-400">No categories matched your search.</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          ...
          <div className="space-y-3">
            {filteredFaqs.map((faq) => (
              <div
                key={faq.id}
                className={`rounded-2xl border backdrop-blur-xl transition-all duration-200 ${openFaq === faq.id ? "border-white/20 bg-white/[0.07]" : "border-white/10 bg-white/[0.04] hover:border-white/15 hover:bg-white/[0.06]"}`}
              >
                <button
                ...
                >
                  <div className="flex-1">
                    <span className="inline-flex rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[10px] font-semibold text-zinc-500 mb-2">{faq.category}</span>
                    <p className="text-sm font-semibold text-white leading-snug">{faq.question}</p>
                  </div>
                  <ChevronDownIcon
                    className={`h-5 w-5 shrink-0 text-zinc-500 mt-1 transition-transform duration-200 ${openFaq === faq.id ? "rotate-180" : ""}`}
                  />
                </button>
                {openFaq === faq.id && (
                    ...
                    <div className="border-t border-white/10 pt-4">
                      ...
                      <div className="mt-5 flex items-center gap-3">
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1 rounded-full border px-3 py-1.5 text-xs transition ${feedback[faq.id] === "up" ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-300" : "border-white/10 bg-white/5 text-zinc-500 hover:text-zinc-300"}`}
                        >
                          <ThumbUpIcon className="h-3.5 w-3.5" /> Yes
                        </button>
                        <button
                          onClick={() => setFeedback((p) => ({ ...p, [faq.id]: p[faq.id] === "down" ? null : "down" }))}
                          className={`inline-flex items-center gap-1 rounded-full border px-3 py-1.5 text-xs transition ${feedback[faq.id] === "down" ? "border-red-400/30 bg-red-400/10 text-red-300" : "border-white/10 bg-white/5 text-zinc-500 hover:text-zinc-300"}`}
                        >
                          <ThumbDownIcon className="h-3.5 w-3.5" /> No
                        </button>
                        {feedback[faq.id] && (
                          <span className="text-xs text-zinc-500">Thanks for your feedback!</span>
                        )}
                      </div>
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {faq.tags.map((tag) => (
                          <button
                            ...
                            className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[10px] text-zinc-600 hover:text-zinc-400 transition"
                          >
                            #{tag}
                          </button>
                        ))}
            ...
            {filteredFaqs.length === 0 && (
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-10 text-center">
                ...
                <p className="text-sm text-zinc-400">No results found. Try different keywords.</p>
          ...
          <section className="relative border-y border-white/5 bg-white/[0.015] py-20">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid gap-8 lg:grid-cols-2">
                  ...
                  <div className="space-y-3">
                    {ivanaTips.map((tip, i) => (
                      <div key={i} className="rounded-xl border border-white/5 bg-black/20 p-4">
                        ...
                        <p className="text-xs leading-5 text-zinc-500">{tip.desc}</p>
                      </div>
                    ))}
                  </div>
                ...
                {/* WhatsApp AI Help */}
                <div className="rounded-[2rem] border border-emerald-500/20 bg-emerald-500/[0.05] p-7 backdrop-blur-xl">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <p className="font-semibold text-white">WhatsApp AI</p>
        ...
        <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="grid gap-5 sm:grid-cols-3">
            <div className="sm:col-span-2 rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 backdrop-blur-xl">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-black/30 mb-5">
              ...
              <div className="mt-6">
                <Link href="/contact" className="group inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10">
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-semibold text-white">Technical Issues</p>
                </div>
                <p className="text-xs leading-5 text-zinc-500">For bugs or platform errors, include your browser, device, and a description of what happened.</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
                <div className="flex items-center gap-2 mb-2">
                  <ZapIcon className="h-4 w-4 text-zinc-500" />
                  <p className="text-sm font-semibold text-white">WhatsApp Issues</p>
                </div>
                <p className="text-xs leading-5 text-zinc-500">Share your Phone Number ID and a description of the issue for fastest resolution.</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
                <div className="flex items-center gap-2 mb-2">
                  <CreditCardIcon className="h-4 w-4 text-zinc-500" />
                  <p className="text-sm font-semibold text-white">Billing Questions</p>
                </div>
                <p className="text-xs leading-5 text-zinc-500">Include your account email and the nature of your billing question for quick lookup.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="relative overflow-hidden px-4 py-20 sm:px-6 lg:px-8">
          ...
          <div className="relative mx-auto max-w-4xl text-center">
            ...
            <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
              ...
              <Link href="/signup" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-7 py-3.5 text-sm font-semibold text-white transition hover:bg-white/10">
                Create Free Account
              </Link>
            </div>
          </div>
      ...
      <footer className="relative z-10 border-t border-white/5 bg-black/50 px-4 py-10 backdrop-blur-xl sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl flex flex-wrap items-center justify-between gap-4 text-xs text-zinc-600">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <Link href="/contact" className="transition hover:text-zinc-400">Contact Support</Link>
