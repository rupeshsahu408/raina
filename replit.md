...
- **Phase 2 — Smart Follow-Up System**: AI detects follow-up needs when an email is opened (calls `POST /inbox/followup/detect`). Shows a card with AI reason + suggested time + one-click "Set Reminder" button in the reading pane. Follow-ups stored in MongoDB `FollowUp` collection. `/inbox/followups` page shows Today / Upcoming / Overdue / Completed tabs with mark-done, reschedule, dismiss, and delete actions. "Follow-Ups" link added to inbox sidebar nav.
