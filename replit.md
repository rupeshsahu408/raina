...
- **Lead & Revenue Intelligence**: Dedicated `/inbox/leads` page scans recent Gmail conversations via `GET /inbox/leads`, detects revenue opportunities, and ranks them as Hot/Warm/Cold with High/Medium/Low buying intent. Backend uses NVIDIA AI for top lead candidates with rule-based fallback, returning opportunity summary, suggested action, reply strategy, confidence, reason, and a ready-to-send reply draft. UI shows pipeline filters, lead cards, detail panel, and Reply Now action through the existing Gmail compose modal. Inbox sidebar includes a Leads entry under AI Features.
