...
- **Recruit AI — Phase 1 India-first job marketplace**: /recruit is a LinkedIn-inspired white landing page for the hiring platform. /recruit/opportunities is a server-rendered public job board with deep filters across niche, work mode, job type, salary, fresher-friendly roles, and verified companies. /recruit/opportunities/[id] is a server-rendered job detail page with an interactive public application form. Public job APIs are under /recruit-public/*; authenticated recruiter APIs are proxied through /backend/recruit/* to avoid conflicts with frontend /recruit/* pages.
